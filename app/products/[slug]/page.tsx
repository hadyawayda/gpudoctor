      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-6 pb-24 lg:grid-cols-[1.1fr_0.9fr] lg:px-10 xl:px-16">
        <div className="space-y-6 rounded-3xl border border-primary-500/20 bg-background-section/70 p-8">
          ...
          <div className="flex flex-wrap gap-3 text-xs text-primary-300/80">
            <span className="rounded-full bg-primary-500/10 px-3 py-1 text-slate-100">{product.brand}</span>
            <span className="rounded-full bg-secondary-500/20 px-3 py-1 text-slate-100">{product.condition}</span>
            <span className="rounded-full bg-teal-400/20 px-3 py-1 text-slate-100">{product.availability}</span>
          </div>
          <p className="text-sm leading-7 text-slate-300">{product.description}</p>
          <div>
            <h2 className="text-lg font-semibold text-primary-300">Key specifications</h2>
            <dl className="mt-4 grid gap-4 text-sm text-slate-300 sm:grid-cols-2">
              {product.specs.map((spec) => (
                <div key={spec.label} className="rounded-2xl border border-primary-500/20 bg-background-card/80 p-4">
                  <dt className="text-xs font-semibold uppercase tracking-wide text-primary-300">{spec.label}</dt>
                  <dd className="mt-2 text-sm text-slate-100/90">{spec.value}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <aside className="space-y-6">
          <div className="rounded-3xl border border-primary-500/20 bg-background-section/70 p-6">
            <h2 className="text-lg font-semibold text-primary-300">Purchase options</h2>
            <p className="mt-3 text-3xl font-semibold text-primary-300">${product.price.toLocaleString()}</p>
          ...
          <div className="rounded-3xl border border-primary-500/20 bg-background-section/70 p-6 text-sm text-slate-300">
            <h2 className="text-lg font-semibold text-primary-300">Service recommendations</h2>
            <ul className="mt-4 space-y-4">
              <li className="rounded-2xl border border-primary-500/15 bg-secondary-500/10 p-4 text-slate-100/90">
                <p className="text-sm font-semibold text-slate-100">Thermal optimisation</p>
                <p className="mt-1 text-xs text-slate-300">
                  Pair with the complete thermal overhaul to refresh paste and pads before reinstalling the card.
                </p>
              </li>
              <li className="rounded-2xl border border-primary-500/15 bg-secondary-500/10 p-4 text-slate-100/90">
                <p className="text-sm font-semibold text-slate-100">Performance re-tuning</p>
                <p className="mt-1 text-xs text-slate-300">Our technicians calibrate fan curves, drivers, and power targets to match your workloads.</p>
              </li>
            </ul>
            <Link
              href="/services"
