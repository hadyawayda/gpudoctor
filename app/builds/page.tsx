    ...
    <main className="bg-background-base text-slate-100">
      <section className="mx-auto max-w-6xl px-6 pb-24 pt-28 lg:px-10 xl:px-16">
        <header className="space-y-6">
          <h1 className="text-4xl font-semibold tracking-tight text-primary-300 sm:text-5xl">Design your GPU build</h1>
        ...
        <div className="mt-12 grid gap-10 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-8 rounded-3xl border border-primary-500/20 bg-background-section/70 p-8">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {categories.map(([category, options]) => (
                <div key={category} className="rounded-2xl border border-primary-500/20 bg-background-card/80 p-5">
                  <label className="text-sm font-semibold text-primary-300" htmlFor={`builder-${category}`}>
                  ...
                  {selectedOptions[category]?.notes ? (
                    <p className="mt-3 text-xs text-slate-400">{selectedOptions[category]?.notes}</p>
                  ) : null}
                </div>
              ))}
            </div>
          ...
          <aside className="flex flex-col gap-6 rounded-3xl border border-primary-500/20 bg-background-section/70 p-8">
            <div>
              <h2 className="text-lg font-semibold text-primary-300">Case preview</h2>
            ...
            <div className="rounded-2xl border border-primary-500/20 bg-background-card/80 p-5 text-sm text-slate-300">
              <h2 className="text-lg font-semibold text-primary-300">Build summary</h2>
              <ul className="mt-4 space-y-3">
                {categories.map(([category]) => {
                  const option = selectedOptions[category]
                  return (
                    <li key={category} className="flex items-start justify-between gap-3 text-xs">
                      <span className="font-semibold text-slate-100">{titleFromCategory(category)}</span>
                      <span className="text-right text-slate-400">{option ? `${option.name} • $${option.price}` : 'Select an option'}</span>
                    </li>
                  )
                })}
              </ul>
