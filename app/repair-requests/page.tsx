    ...
    <main className="bg-background-base text-slate-100">
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-6 pb-20 lg:grid-cols-[1.15fr_0.85fr] lg:px-10 xl:px-16">
        <form className="space-y-6 rounded-3xl border border-primary-500/20 bg-background-section/70 p-8 text-sm text-slate-300">
          <div>
            <label className="text-sm font-semibold text-primary-300" htmlFor="device">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <label className="text-sm font-semibold text-primary-300" htmlFor="symptoms">
                Primary symptom
              </label>
              <select
                id="symptoms"
            ...
            <div>
              <label className="text-sm font-semibold text-primary-300" htmlFor="urgency">
                Turnaround target
              </label>
              <select
                id="urgency"
          ...
          <div>
            <label className="text-sm font-semibold text-primary-300" htmlFor="service">
          ...
          <div>
            <label className="text-sm font-semibold text-primary-300" htmlFor="history">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <label className="text-sm font-semibold text-primary-300" htmlFor="pickup">
                Pickup preference
              </label>
              <select
                id="pickup"
            ...
            <div>
              <label className="text-sm font-semibold text-primary-300" htmlFor="notifications">
                Notify me via
              </label>
              <select
                id="notifications"
        ...
        <aside className="space-y-8">
          <div className="rounded-3xl border border-primary-500/20 bg-background-section/70 p-6">
            <h2 className="text-lg font-semibold text-primary-300">Track each milestone</h2>
          ...
          <div className="rounded-3xl border border-primary-500/20 bg-background-section/70 p-6 text-sm text-slate-300">
            <h2 className="text-lg font-semibold text-primary-300">Recent repair history</h2>
            <ul className="mt-4 space-y-4">
              {repairHistory.map(({ id, device, service, status, cost }) => (
                <li key={id} className="rounded-2xl border border-primary-500/15 bg-background-card/80 p-4">
                  <div className="flex items-center justify-between text-xs text-primary-300/70">
                    <span>{id}</span>
                    <span>{status}</span>
                  </div>
                  <p className="mt-2 text-sm font-semibold text-slate-100">{device}</p>
                  <p className="mt-1 text-xs text-primary-300/70">{service}</p>
                  <p className="mt-3 text-sm font-semibold text-primary-300">{cost}</p>
                </li>
              ))}
            </ul>
            <Link
