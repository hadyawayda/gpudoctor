    ...
    <section className="mx-auto max-w-6xl px-6 pb-16 lg:px-10 xl:px-16">
      <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr]">
        <form className="space-y-6 rounded-3xl border border-primary-500/20 bg-background-section/70 p-6 text-sm text-slate-100/80">
          <div>
            <label className="text-sm font-semibold text-primary-300" htmlFor="search">
              Search listings
            </label>
            <input
              id="search"
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="text-sm font-semibold text-primary-300" htmlFor="cooler">
                Cooler type
              </label>
              <select
                id="cooler"
            ...
            <div>
              <label className="text-sm font-semibold text-primary-300" htmlFor="fans">
                Number of fans
              </label>
              <select
                id="fans"
            ...
            <div>
              <label className="text-sm font-semibold text-primary-300" htmlFor="memory">
                Memory capacity
              </label>
              <select
                id="memory"
          ...
          <div className="space-y-3 rounded-2xl border border-primary-500/20 bg-background-card p-4 text-xs text-slate-100/80">
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={onlyCertified}
                onChange={(event) => setOnlyCertified(event.target.checked)}
                className="h-4 w-4 rounded border border-primary-500/40 bg-transparent"
              />
        ...
        <div className="space-y-6">
          <header className="flex flex-col gap-3 rounded-3xl border border-primary-500/20 bg-background-section/70 p-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-primary-300">{filteredListings.length} listings</h2>
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            {filteredListings.map(({ id, title, brand: brandName, model, price, condition, description, stats, location, refurbishedByDoctor, includesRefurbOption, coolerType: type, fanCount: fans, memory }) => (
              <article key={id} className="rounded-3xl border border-primary-500/20 bg-background-section/70 p-6">
                <header className="space-y-1">
                  ...
                  <p className="text-sm text-slate-100/80">{model}</p>
                </header>
                <p className="mt-4 text-sm leading-6 text-slate-100/75">{description}</p>
                <p className="mt-4 text-xs text-primary-300/80">{stats}</p>
                <div className="mt-4 grid gap-2 text-xs text-slate-100/75">
                ...
                <div className="mt-4 flex flex-wrap gap-3 text-xs text-slate-100/75">
                  <span className="rounded-full bg-primary-500/10 px-3 py-1">{location}</span>
                  {refurbishedByDoctor ? (
                    <span className="rounded-full bg-secondary-500/20 px-3 py-1 text-primary-200">GPU Doctor Certified</span>
                  ) : null}
                  {includesRefurbOption ? (
                    <span className="rounded-full bg-teal-400/20 px-3 py-1 text-primary-200">Refurbish add-on available</span>
                  ) : null}
                </div>
                <div className="mt-6 flex items-center justify-between">
                  <span className="text-lg font-semibold text-primary-300">{price}</span>
                  <Link href={`/repair-requests?listing=${id}`} className="text-xs font-semibold text-primary-200 hover:text-slate-100">
                    Request diagnostics →
                  </Link>
                </div>
              </article>
