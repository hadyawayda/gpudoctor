...
export const metadata: Metadata = {
  title: "Products",
  description:
    "Browse GPUs, CPUs, cooling, and chassis curated by GPU Doctor engineers with telemetry-backed validation.",
};
      ...
      <section className="mx-auto max-w-6xl px-6 pb-24 lg:px-10 xl:px-16">
        <div className="grid gap-10">
          {productCategories.map(
            ({ title, subtitle, categories, highlight }) => {
              const products = getProductsByCategories(categories);
              return (
                <section
                  key={title}
                  className="space-y-6 rounded-3xl border border-primary-500/20 bg-background-section/70 p-8"
                >
      ...
      <section className="bg-background-base pb-24">
        <div className="mx-auto max-w-4xl rounded-3xl border border-accent-500/20 bg-accent-500/10 p-8 text-center">
          ...
          <p className="mt-4 text-sm text-slate-300">
            Our engineers can source rare GPUs, pre-flash firmware for cluster
            deployments, or assemble prebuilt rigs. Share your wishlist and
            we&apos;ll prepare a documented parts list, turnaround estimate, and
            warranty options tailored to your use case.
          </p>
