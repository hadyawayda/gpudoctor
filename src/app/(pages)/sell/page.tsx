...
import { catalogProducts } from "@data/site";

const gpuOptions = catalogProducts.filter(
  (product) => product.category === "GPU"
);
const uniqueBrands = Array.from(
  new Set(gpuOptions.map((product) => product.brand))
).sort();
...
const SellPage = () => {
  ...
  const modelsForBrand = useMemo(
    () => gpuOptions.filter((product) => product.brand === formState.brand),
    [formState.brand]
  );

  const estimatedValue = useMemo(() => {
    const basePrice =
      modelsForBrand.find((product) => product.model === formState.model)
        ?.price ?? 500;
    ...
    return Math.round(
      basePrice * conditionModifier * issueModifier * refurbishModifier
    );
  }, [
    formState.condition,
    formState.issue,
  ...
  const handleChange = (
    ...
    setFormState((state) => ({
      ...
      ...(name === "brand"
        ? {
            model:
              gpuOptions.find((product) => product.brand === value)?.model ??
              state.model,
          }
        : null),
    }));
    ...
    <main className="bg-background-base text-slate-100">
      <section className="mx-auto max-w-5xl px-6 pb-24 pt-28 lg:px-10 xl:px-16">
        <header className="space-y-6 text-balance">
          <h1 className="text-4xl font-semibold tracking-tight text-secondary-300 sm:text-5xl">
            Sell or consign your GPU
          </h1>
          <p className="text-lg text-slate-300">
            Tell us about your graphics card and choose whether you want a
            refurbish package. We&apos;ll generate an instant trade-in estimate,
            schedule a pickup, and list it on the GPU Doctor marketplace once it
            passes validation.
          </p>
        </header>
        ...
        >
          <div className="space-y-6">
            ...
            <div>
              ...
              >
                <option value="yes">
                  Yes – add GPU Doctor refurbish package
                </option>
          ...
          <aside className="flex flex-col justify-between rounded-3xl border border-primary-500/20 bg-background-card p-6 text-sm text-slate-300">
            <div>
              ...
              <p className="mt-2 text-xs text-slate-400">
                Final payout confirmed after diagnostics. Refurbish packages can
                increase value up to 15%.
              </p>
            </div>
            <div className="space-y-4">
              <div className="rounded-2xl border border-primary-500/20 bg-background-section/70 p-4 text-xs text-slate-300">
                <p>
                  • Free insured pickup within 24 hours in supported cities.
                </p>
                <p className="mt-2">
                  • Instant marketplace listing once refurbished or validated.
                </p>
                <p className="mt-2">
                  • Track status and offers directly from your GPU Doctor
                  dashboard.
                </p>
              </div>
              {submitted ? (
                <div className="rounded-2xl border border-primary-500/30 bg-primary-500/10 p-4 text-xs text-primary-300">
                  Thank you! A specialist will confirm shipping details via
                  email shortly.
                </div>
              ) : null}
            </div>
