...
const StoreContent = ({ products }: { products: CatalogProduct[] }) => {
  ...
  const brands = useMemo(
    () => unique(products.map((product) => product.brand)).sort(),
    [products]
  );
  const categories = useMemo(
    () => unique(products.map((product) => product.category)).sort(),
    [products]
  );
          ...
          <header className="flex flex-col gap-3 rounded-3xl border border-primary-500/20 bg-background-section/70 p-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-100/70">
                These listings are stocked by GPU Doctor and include
                documentation for warranty claims and resale. Add optional
                installation or maintenance services at checkout.
              </p>
            </div>
            <Link
              href="/services"
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <h2 className="mt-3 text-lg font-semibold text-primary-200">
                  {product.name}
                </h2>
