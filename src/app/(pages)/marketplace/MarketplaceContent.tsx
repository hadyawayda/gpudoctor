...
import type { MarketplaceListing } from "@data/site";
...
const MarketplaceContent = ({
  ...
  const [coolerType, setCoolerType] = useState<
    "All" | MarketplaceListing["coolerType"]
  >("All");
  ...
  const prices = useMemo(
    () => listings.map((item) => item.priceValue),
    [listings]
  );
  ...
  const brands = useMemo(
    () => unique(listings.map((item) => item.brand)).sort(),
    [listings]
  );
  const fanOptions = useMemo(
    () => unique(listings.map((item) => item.fanCount)).sort((a, b) => a - b),
    [listings]
  );
  ...
  const filteredListings = useMemo(() => {
    return listings.filter((listing) => {
      if (
        listing.priceValue < priceRange[0] ||
        listing.priceValue > priceRange[1]
      ) {
        return false;
      }
      if (query) {
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                value={coolerType}
                onChange={(event) =>
                  setCoolerType(event.target.value as typeof coolerType)
                }
                className="mt-2 w-full rounded-2xl border border-primary-500/20 bg-background-card px-4 py-3 text-sm text-slate-100"
              >
          ...
          <div>
            ...
            <div className="mt-4 flex items-center gap-4">
              <input
                ...
                value={priceRange[0]}
                onChange={(event) =>
                  handlePriceChange(0)(Number(event.target.value))
                }
                className="w-full"
              />
              <input
                ...
                value={priceRange[1]}
                onChange={(event) =>
                  handlePriceChange(1)(Number(event.target.value))
                }
                className="w-full"
              />
            </div>
          ...
          <header className="flex flex-col gap-3 rounded-3xl border border-primary-500/20 bg-background-section/70 p-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-100/70">
                All marketplace GPUs include documented diagnostics. Add a
                refurbish package before shipping to receive certification
            ...
            {filteredListings.map(
              ({
                ...
                >
                  ...
                  <p className="mt-4 text-sm leading-6 text-slate-100/75">
                    {description}
                  </p>
                  ...
                  <div className="mt-4 grid gap-2 text-xs text-slate-100/75">
                    ...
                    <span>
                      Fans: {fans === 0 ? "Water cooled" : `${fans}-fan array`}
                    </span>
                    <span>Memory: {memory}</span>
                  </div>
                  <div className="mt-4 flex flex-wrap gap-3 text-xs text-slate-100/75">
                    <span className="rounded-full bg-primary-500/10 px-3 py-1">
                      {location}
                    </span>
                    {refurbishedByDoctor ? (
                      <span className="rounded-full bg-secondary-500/20 px-3 py-1 text-primary-200">
                        GPU Doctor Certified
                      </span>
                    ) : null}
                    {includesRefurbOption ? (
                      <span className="rounded-full bg-teal-400/20 px-3 py-1 text-primary-200">
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    ...
                    <Link
                      href={`/repair-requests?listing=${id}`}
                      className="text-xs font-semibold text-primary-200 hover:text-slate-100"
                    >
                      Request diagnostics →
                    </Link>
                  </div>
